...
exports.getAvailableWorkers = asyncHandler(async (req, res, next) => {
  ...
  const filteredWorkers = await Worker.find(JSON.parse(queryStr))
    .populate({
      path: "user",
      select: "name",
    })
  ...
  if (!filteredWorkers) {
    return next(new ErrorResponse("No workers found", 404));
  }
  ...
  for (let i = 0; i < filteredWorkers.length; i++) {
    for (let j = 0; j < filteredWorkers[i].schedule.length; j++) {
      if (
        filteredWorkers[i].schedule[j].date.toDateString() === reqDate &&
...
exports.uploadWorkerPhoto = asyncHandler(async (req, res, next) => {
  ...
  req.file.name = `photo_${req.params.workerId}${
